...
class ErrorLogger:
    def __init__(self, folder_loc) -> None:
        ...
        self.lock = Lock()
        self.workers = [Thread(target=self.logEvent, args=(f"error_logging_thread_{idx+1}",)) for idx in range(THREAD_COUNT)]
        self.end = False
        if not os.path.isdir(folder_loc):
            try:
                os.mkdir(folder_loc)
            except FileNotFoundError:
                raise Exception("Invalid folder path passed to error logger")
        try:
            self.log_file = open(folder_loc + f"/error_log_file_{datetime.now().strftime('%d_%m_%Y_%H_%M_%S')}.txt", "w")
